...
class MiRoClient:
    ...
    def __init__(self):
        ...
        self.reward = 0
        self.action_to_do = 7  # Current action index
    ...
    def loop(self):
        ...
        while not rospy.core.is_shutdown():
            ...
            distance = self.actions[10]()
            print(distance)
            
            # If close to hostile object, move backwards and turn.
            if distance[0] < 0.5 and distance[0] > -1:
                # Move backwards.
                start_time = rospy.Time.now()
                self.actions[6](start_time)
                # Turn.
                start_time = rospy.Time.now()
                self.actions[2](start_time)
            elif thirst > 10:
                # Turn to face the object that restores this.
                start_time = rospy.Time.now()
                self.actions[7](start_time,self.colours[2])
                # Calculate distance from object.
                distance = self.actions[10]()
                print("DISTANCE:", distance[2])
                while distance[2] > 0.2:
                    print ("loop")
                    # Move forward.
                    start_time = rospy.Time.now()
                    self.actions[5](start_time)
                    # Turn to face the object that restores this.
                    start_time = rospy.Time.now()
                    self.actions[7](start_time,self.colours[2])
                    # Calculate distance from object.
                ...
                start_time = rospy.Time.now()
                self.actions[7](start_time,self.colours[1])
                # Calculate distance from object.
                start_time = rospy.Time.now()
                distance = self.actions[10]()
